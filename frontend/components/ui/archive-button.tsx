...
const ArchiveButton = forwardRef<HTMLButtonElement, ArchiveButtonProps>(
  (
    {
      className,
      variant,
          ...
          {loading ? (
            <>
              <span className="flex items-center justify-center gap-[inherit] opacity-0">
                {LeadingIcon && !isIconOnly ? <LeadingIcon size={iconSize} strokeWidth={2} /> : null}
                {children}
                {TrailingIcon && !isIconOnly ? <TrailingIcon size={iconSize} strokeWidth={2} /> : null}
              </span>
...
export { ArchiveButton, buttonVariants };
